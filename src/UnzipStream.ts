...
import MatcherStream from './MatcherStream';
...
export interface UnzipStreamOpts {
  debug: boolean;
...
export interface ParsedEntity {
  ...
  centralDirectoryRecordSize: number;
  commentLength: number;
  flags: number;
  uncompressedSize: number;
}

export interface OutStreamInfo {
  stream?: PassThrough | MatcherStream;
  limit?: number;
  written?: number;
}
...
class UnzipStream extends Transform {
  ...
  outStreamInfo: OutStreamInfo;

  _drainAllEntries: boolean;
  ...
  _readExtraFields(data) {
    ...
    const result = { parsed: extra, debug: null };
  ...
  _decodeString(buffer: Buffer, isUtf8?: boolean): string {
    if (isUtf8) {
      return buffer.toString('utf8');
    }
    // allow passing custom decoder
    if (this.opts.decodeString) {
      return this.opts.decodeString(buffer);
    }
    let result = '';
  ...
  _parseOrOutput(encoding: BufferEncoding, cb: () => void) {
    let consume;
